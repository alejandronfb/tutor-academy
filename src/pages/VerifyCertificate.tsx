...
export default function VerifyCertificate() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { ...cert, tutorName: profile?.full_name || "Unknown Tutor" };
    },
    enabled: !!id,
  });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-secondary/30 px-4">
      <div className="w-full max-w-md rounded-xl border bg-card p-8 shadow-card text-center">
        ...
        <h1 className="text-xl font-bold text-foreground mb-2">Certificate Verified</h1>
        <div className="rounded-lg bg-emerald-50 dark:bg-emerald-950/20 p-4 mb-4">
          <div className="flex items-center justify-center gap-2 text-emerald-700 dark:text-emerald-400 mb-2">
            ...
            <span className="font-semibold text-sm">This certificate is valid</span>
          </div>
        </div>
        <div className="space-y-2 text-sm">
          <div><span className="text-muted-foreground">Tutor:</span> <span className="font-medium text-foreground">{data.tutorName}</span></div>
          <div><span className="text-muted-foreground">Certificate:</span> <span className="font-medium text-foreground">{data.title}</span></div>
          ...
          <div><span className="text-muted-foreground">Verification ID:</span> <span className="font-mono text-xs text-foreground">{data.verification_id}</span></div>
        </div>
      </div>
    </div>
  );
